...
await p.keyboard.press('m'); await p.mouse.click(340,180); await p.waitForTimeout(300)
...
await p.mouse.move(mb.x+mb.width-6, mb.y+mb.height-6); await p.mouse.down(); await p.mouse.move(mb.x+mb.width+380, mb.y+mb.height+380,{steps:15}); await p.mouse.up(); await p.waitForTimeout(400)
...
let bxs=[]; for(let i=0;i<await f.count();i++) bxs.push({i,...(await f.nth(i).boundingBox())})
bxs.sort((a,b)=>a.y-b.y); const f0=bxs[0],f1=bxs[1],f2=bxs[2]
// insert f2 between f0 and f1, then DROP
await p.mouse.move(f2.x+f2.width/2, f2.y+f2.height/2); await p.mouse.down()
await p.mouse.move(f0.x+f0.width/2, (f0.y+f0.height+f1.y)/2, {steps:20}); await p.waitForTimeout(200); await p.mouse.up(); await p.waitForTimeout(500)
// read final stack
let after=[]; for(let i=0;i<await f.count();i++) after.push(await f.nth(i).boundingBox())
after.sort((a,b)=>a.y-b.y)
R.count=after.length
// no overlap: each next starts below previous bottom
R.gaps = after.slice(1).map((c,i)=>Math.round(c.y-(after[i].y+after[i].height)))
R.noOverlap = R.gaps.every(g=>g>=-2)
R.errs=errs.slice(0,3)
await p.screenshot({path:new URL('./insertdrop.png',import.meta.url).pathname, clip:{x:Math.max(0,mb.x-30),y:Math.max(0,mb.y-30),width:520,height:620}})
console.log('INSERTDROP '+JSON.stringify(R))
await b.close()
