...
// Édition contexte/contraintes/estimations ici ; le reste sur le canvas. Textes interpolés (aucun v-html).
import { computed, ref, watch } from 'vue'
import { useProjectStore } from '@/stores/project'
import { useUiStore } from '@/stores/ui'
import type { SpecsFilter } from '@/domain/derive/specs'
import type { AttentionPoint } from '@/domain/attention'
import { exportSpecsMarkdown } from '@/io/export/markdown'
...
async function onExportMarkdown(): Promise<void> {
  await saveTextAs(`${slugForFile()}-specs.md`, exportSpecsMarkdown(project.doc), 'text/markdown')
}
...
<template>
  <div class="specs-view flex h-full bg-paper font-sans2 text-slate-700">
    ...
    <div class="doc-scroll flex-1 overflow-auto">
      ...
      <header
        class="no-print sticky top-0 z-20 flex flex-wrap items-center gap-3 border-b border-slate-200/70 bg-paper/85 px-8 py-3 backdrop-blur"
      >
      ...
      <div class="mx-auto grid max-w-[72rem] grid-cols-1 gap-x-12 px-8 py-12 lg:grid-cols-[210px_minmax(0,1fr)]">
                ...
                <table class="w-full border-collapse text-sm">
                  <thead>
                    <tr class="bg-slate-50/70 text-left font-mono2 text-[10px] uppercase tracking-wide text-slate-400">
                      ...
                      <th class="px-4 py-2 font-medium">Périmètre</th>
                  ...
                  <tbody>
                    <tr v-for="f in group.features" :key="f.id" class="border-t border-slate-100 align-middle transition-colors hover:bg-slate-50/50">
                      ...
                      <td class="px-4 py-2 text-slate-500">{{ perimeterLabel(f.perimeter) }}</td>
                      <td class="px-4 py-2">
                        <input
                          :value="f.estimate"
                          class="w-20 rounded-md border border-transparent bg-transparent px-1.5 py-0.5 font-mono2 text-xs text-slate-600 outline-none transition hover:border-slate-200 focus:border-emerald-400 focus:bg-white focus:ring-1 focus:ring-emerald-300"
            ...
            >
              <div class="border-b border-slate-100 bg-slate-50/50 px-6 py-4">
                <h3 class="flex flex-wrap items-center gap-2">
                  ...
                  <span v-if="page.page.attrs.route" class="rounded-md bg-white px-2 py-0.5 font-mono2 text-[11px] text-slate-500 ring-1 ring-slate-200">{{ page.page.attrs.route }}</span>
              ...
              <div class="px-6 py-4">
                ...
                <div v-for="b in page.blocks" :id="`el-${b.block.id}`" :key="b.block.id" class="scroll-mt-24 border-b border-slate-100 py-3 first:pt-0 last:border-0 last:pb-0">
                  ...
                  <p v-if="b.block.attrs.description" class="mt-1 whitespace-pre-line text-sm text-slate-600">{{ b.block.attrs.description }}</p>
                  ...
                  <ul class="mt-2 space-y-1.5 text-sm">
                    <li v-for="(c, i) in b.constraints" :key="'c' + i" class="text-slate-500"><span class="font-mono2 text-[10px] uppercase tracking-wide text-slate-400">Contrainte</span> — {{ c }}</li>
                    <li v-for="beh in b.behaviors" :id="`el-${beh.id}`" :key="beh.id" class="scroll-mt-24">
                      ...
                      <span v-if="beh.attrs.facet" class="ml-1 rounded bg-slate-100 px-1 font-mono2 text-[9px] uppercase text-slate-400">{{ beh.attrs.facet }}</span>
                      ...
                      <span class="ml-1 inline-flex items-center gap-1">
                        <input
                          ...
                          class="w-14 rounded-md border border-transparent bg-transparent px-1 py-0.5 font-mono2 text-xs text-slate-600 outline-none transition hover:border-slate-200 focus:border-emerald-400 focus:bg-white focus:ring-1 focus:ring-emerald-300"
                    ...
                    <li v-for="api in b.apis" :id="`el-${api.noteId}`" :key="api.noteId" class="scroll-mt-24 text-slate-500">
                      <span class="font-mono2 text-[10px] uppercase tracking-wide text-slate-400">API</span>
                      <button type="button" class="ml-1 font-medium text-cyan-700 decoration-cyan-200 underline-offset-2 hover:underline" @click="goCanvas(api.noteId)">{{ api.serviceName }}</button>
                      <span class="font-mono2 text-xs text-slate-500"> {{ api.method }} {{ api.path }}</span>
                      <button v-if="isIncomplete(api.noteId)" type="button" class="badge-warn ml-1 ring-1 ring-amber-200 bg-amber-50 text-amber-600" title="À compléter" @click="goCanvas(api.noteId)">incomplet</button>
                    </li>
                  </ul>
                </div>
                ...
                <div v-if="page.apis.length" class="border-t border-slate-100 pt-3">
                  ...
                  <ul class="mt-1.5 space-y-1 text-sm">
                    <li v-for="api in page.apis" :id="`el-${api.noteId}`" :key="api.noteId" class="scroll-mt-24 text-slate-500">
                      <button type="button" class="font-medium text-cyan-700 decoration-cyan-200 underline-offset-2 hover:underline" @click="goCanvas(api.noteId)">{{ api.serviceName }}</button>
