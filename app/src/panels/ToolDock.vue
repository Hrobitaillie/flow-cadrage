...
// Dock d'outils vertical (gauche-centre) v2 : sélection V, page P, bloc B, note comportement N,
// note API A, lien nav L. Un seul outil actif à la fois. La création retourne auto à « sélection »,
// sauf en série (⇧ maintenu au clic) — géré par runTool. Le service se crée depuis la note API
// (pas d'outil dédié). (evolution-v2.md §2, interface.md §ToolDock)
...
const icons: Record<ToolId, string> = {
  ...
  page: 'M5 3h10v14H5z',
  block: 'M4 8h12v6H4z',
  behavior: 'M4 5h12v7H9l-3 3v-3H4z',
  api: 'M7 5l-4 5 4 5M13 5l4 5-4 5',
...
const separators = computed(() => new Set<ToolId>(['select', 'api', 'feature'])) // groupes visuels
    ...
    <div class="flex flex-col gap-1 p-1.5">
      <template v-for="t in tools" :key="t.id">
        <button
          ...
          class="group relative flex h-9 w-9 items-center justify-center rounded-lg transition-colors duration-150 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-500"
          :class="
            tool.active === t.id
        ...
        >
          <span v-if="t.id === 'api'" class="text-[10px] font-bold tracking-tight" aria-hidden="true">API</span>
          <svg v-else viewBox="0 0 20 20" class="h-5 w-5" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linejoin="round" stroke-linecap="round" aria-hidden="true">
            <path :d="icons[t.id]" />
          </svg>
          <span v-if="t.id !== 'api'" class="pointer-events-none absolute -bottom-0.5 right-0.5 text-[8px] font-semibold opacity-60">{{ t.key.toUpperCase() }}</span>
        </button>
